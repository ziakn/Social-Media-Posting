            ...
            <div className="container mx-auto px-6 max-w-[1280px]">
                <div className="text-center mb-24 space-y-6">
                    <h2 className="text-4xl md:text-5xl font-extrabold text-[#0C1B33] font-plus-jakarta tracking-tight leading-tight uppercase">
                        Simple, <span className="text-[#00A2FF]">Transparent</span> Protocol
                    </h2>
                ...
                <div className="grid md:grid-cols-3 gap-10">
                    {plans.map((plan, i) => (
                        <div key={i} className={`p-12 rounded-[10px] border flex flex-col transition-all duration-500 relative group ${plan.popular ? 'bg-[#0C1B33] border-[#0C1B33] text-white shadow-2xl -translate-y-4' : 'bg-white border-slate-200 text-[#0C1B33] shadow-sm hover:shadow-subtle hover:border-[#00A2FF]/30'}`}>
                            ...
                            <ul className="space-y-6 mb-12 flex-1">
                                {plan.features.map((f, j) => (
                                    <li key={j} className="flex items-center gap-4 text-xs font-bold font-inter group/item">
                                        <Check className={`h-4 w-4 shrink-0 transition-transform group-hover/item:scale-150 ${plan.popular ? 'text-[#F9C80E]' : 'text-[#00A2FF]'}`} />
                ...
                <div className="mt-20 text-center">
                    <Link href="/pricing" className="inline-flex items-center gap-2 text-[10px] font-black text-[#00A2FF] uppercase tracking-widest hover:underline font-plus-jakarta">
