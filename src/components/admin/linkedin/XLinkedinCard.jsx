...
export default function XLinkedinCard({
    post,
    onAction, // (post, action) => {}
    publishingId
}) {
    const isPublishing = publishingId === post.id;
    const media = post.mediaUrls || (post.mediaUrl ? [{ url: post.mediaUrl, type: post.mediaType || post.postType }] : []) || (post.imageUrl ? [{ url: post.imageUrl, type: 'image' }] : (post.videoUrl ? [{ url: post.videoUrl, type: 'video' }] : []));
    const message = post.text || post.message || post.content?.text || post.caption || "";

    const getStatusBadge = (status) => {
        switch (status) {
            case 'posted':
            case 'published': return <Badge className="bg-green-500/80 text-white border-0 text-[8px] font-black uppercase tracking-tighter">Published</Badge>;
            case 'scheduled': return <Badge className="bg-blue-500/80 text-white border-0 text-[8px] font-black uppercase tracking-tighter">Scheduled</Badge>;
            case 'failed': return <Badge className="bg-red-500/80 text-white border-0 text-[8px] font-black uppercase tracking-tighter">Failed</Badge>;
            default: return null;
        }
    };

    return (
        <Card className="group relative aspect-square overflow-hidden rounded-2xl border-0 shadow-lg hover:shadow-2xl transition-all duration-500 bg-gray-900 font-sans">
            {/* Media Content */}
            <div className="absolute inset-0 z-0">
                {media.length > 0 ? (
                    media[0].type?.startsWith('video') || post.mediaType === 'VIDEO' || post.videoUrl ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-[#0077b5]/10 to-[#0077b5]/30 p-6 text-center">
                        <LinkedinLogo className="h-10 w-10 text-[#0077b5]/40 mb-3" />
                        <p className="text-[11px] font-bold text-[#0077b5]/60 line-clamp-3 leading-relaxed">{message || "Professional update"}</p>
                    </div>
                )}
            </div>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-44 rounded-2xl border-none shadow-2xl p-1.5 focus:ring-0">
                            {post.status === 'posted' || post.status === 'published' ? (
                                <>
                                    <DropdownMenuItem onClick={() => onAction?.(post, 'analytics')} className="rounded-xl font-bold py-2 gap-2 text-[#0077b5] px-3">
                                ...
                                <>
                                    ...
                                    <DropdownMenuItem onClick={() => onAction?.(post, 'publish_now')} className="rounded-xl font-bold py-2 gap-2 text-[#0077b5] px-3">
                ...
                <div className="space-y-3 translate-y-[20px] group-hover:translate-y-0 transition-transform duration-300">
                    <div className="flex items-center gap-4 text-white">
                        <div className="flex items-center gap-1.5">
                            <Heart className="h-4 w-4 fill-[#0077b5] text-[#0077b5]" />
                            <span className="text-sm font-black tracking-tight">{formatNumber(post.metrics?.likes || 0)}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            <MessageCircle className="h-4 w-4 fill-white text-white opacity-80" />
                            <span className="text-sm font-black tracking-tight">{formatNumber(post.metrics?.comments || 0)}</span>
                        </div>
                    </div>
            ...
            {isPublishing && (
                <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-white italic">Publishing...</span>
                </div>
            )}

            {/* Platform Badge */}
            <div className="absolute top-4 right-4 z-0 group-hover:opacity-0 transition-opacity p-1.5 bg-[#0077b5]/80 backdrop-blur-md rounded-lg border border-white/10">
                <LinkedinLogo className="h-3 w-3 text-white" />
            </div>
        </Card>
    );
}
