...
import BlueSkyFullCalendar from "./BlueSkyFullCalendar";
import { getAllBlueSkyCalendarPosts } from "@/app/actions/social/bluesky/blueskyPostsActions";
import { getUserBlueSkyAccounts } from "@/app/actions/social/bluesky/createPost";

export default function BlueSkyCalendarViewComponent({
    onDateClick,
    onPostClick,
    onRefresh,
    refreshTrigger = 0
    ...
    useEffect(() => {
        // Fetch accounts once
        getUserBlueSkyAccounts().then(res => {
    ...
    useEffect(() => {
        ...
        getAllBlueSkyCalendarPosts({ startDate, endDate }).then(res => {
