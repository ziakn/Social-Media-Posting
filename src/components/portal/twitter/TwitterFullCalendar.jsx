...
import { ChevronLeft, ChevronRight, Plus, CheckCircle2, Clock, Globe, Eye, Edit, Trash2, Send, Loader2, BarChart3, Twitter, Repeat2 } from "lucide-react";
...
import { publishTwitterPostNow } from "@/app/actions/social/twitter/twitterPostsActions";
import { toast } from "sonner";

export default function TwitterFullCalendar({
    posts = [],
    onDateClick,
    onPostClick,
    onMonthChange,
    ...
    const handlePublishNow = async (e, post) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                toast.error(result.message || "Failed to publish tweet");
            }
        } catch (error) {
                ...
                <div className="flex flex-col">
                    ...
                    {!mini && (
                        <p className="text-gray-500 text-sm mt-1">
                            Manage and plan your Twitter content
                        </p>
                    )}
                </div>
                    ...
                    return (
                        <div key={day.toString()} className={cn("bg-white p-2 transition-all duration-200 relative group flex flex-col", mini ? "min-h-[70px]" : "min-h-[140px]", !isSelectedMonth && "bg-gray-50/50", "hover:bg-gray-50/80 cursor-default")}>
                            ...
                            <div className={cn("flex-1 space-y-1 overflow-y-auto scrollbar-hide text-blue-600", mini ? "max-h-[50px]" : "max-h-[100px]")}>
                                {dayPosts.map((post) => {
                                    const isPublished = post.status === "posted" || post.isPublished;
                                                ...
                                                <div
                                                    className={cn(
                                                        ...
                                                        post.postType === "video" ? "border-purple-200 bg-purple-50/30" :
                                                            post.postType === "image" ? "border-blue-200 bg-blue-50/30" :
