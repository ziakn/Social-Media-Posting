...
import { publishPinterestPostNow } from "@/app/actions/social/pinterest/pinterestPostsActions";
import { toast } from "sonner";
import PinterestLogo from "@/components/icons/PinterestLogo";
import PinterestPreview from "./PinterestPreview";

export default function PinterestFullCalendar({
    posts = [],
    accounts = [],
    onDateClick,
    onPostClick,
                ...
                <div className="flex flex-col">
                    ...
                    {!mini && (
                        <p className="text-gray-500 text-sm mt-1">
                            Plan and manage your Pinterest Pins
                        </p>
                    )}
                </div>
                            ...
                            <div className={cn("flex items-center justify-between mb-1", mini && "mb-0.5")}>
                                <span
                                    className={cn(
                                        ...
                                        isTodayDate
                                            ? "bg-[#E60023] text-white shadow-md ring-2 ring-red-50"
                                            : isSelectedMonth
                                ...
                                {onDateClick && !isBefore(day, startOfToday()) && (
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className={cn("opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-full hover:bg-red-50 hover:text-[#E60023]", mini ? "h-5 w-5" : "h-7 w-7")}
                                                    ...
                                                    <div className={cn("relative flex-shrink-0 rounded-md overflow-hidden bg-gray-100 border border-gray-200", mini ? "w-4 h-4" : "w-7 h-7")}>
                                                        ...
                                                        {isProcessing && (
                                                            <div className="absolute inset-0 bg-white/60 flex items-center justify-center">
                                                                <Loader2 className="h-3 w-3 animate-spin text-[#E60023]" />
                                                            </div>
                                                        )}
                                                    </div>
