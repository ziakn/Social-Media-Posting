...
import { fetchThreadsPosts } from "@/app/actions/social/threads/getPosts";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Spinner } from "@/components/ui/spinner";
import { AtSign, ExternalLink, RefreshCcw, MessageSquare, Heart } from "lucide-react";

export default function PublishedThreadsPosts() {
    ...
    const loadPosts = async () => {
        setLoading(true);
        try {
            const res = await fetchThreadsPosts("published");
            if (res.success) {
                setPosts(res.posts || []);
            }
        } catch (err) {
        ...
        return (
            <Card className="border-dashed border-neutral-300">
                <CardContent className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground mb-6">You haven't published any posts to Threads yet.</p>
                        ...
                        <CardContent className="p-5 space-y-4">
                            ...
                            {post.content.mediaUrl && (
                                <div className="rounded-lg overflow-hidden border border-neutral-100 bg-neutral-50 aspect-video">
                                    {post.content.mediaType === "IMAGE" ? (
                                        <img src={post.content.mediaUrl} alt="Threads media" className="w-full h-full object-cover" />
                                    ) : (
                            ...
                            <div className="pt-4 border-t border-neutral-100 flex items-center justify-between">
                                ...
                                <Button size="sm" variant="ghost" className="h-8 px-2 text-neutral-600" asChild>
                                    <a href={`https://www.threads.net/t/${post.threadsPostId}`} target="_blank" rel="noopener noreferrer">
