...
import { publishLinkedinPostNow } from "@/app/actions/social/linkedin/linkedinPostsActions";
import { toast } from "sonner";
import { LinkedinLogo } from "@/components/icons/LinkedinLogo";
import LinkedinPreview from "./LinkedinPreview";

export default function LinkedinFullCalendar({
    posts = [],
    accounts = [],
    onDateClick,
    onPostClick,
                ...
                <div className="flex flex-col">
                    ...
                    {!mini && (
                        <p className="text-gray-500 text-sm mt-1">
                            Plan and manage your professional presence
                        </p>
                    )}
                </div>
                            ...
                            <div className={cn("flex items-center justify-between mb-1", mini && "mb-0.5")}>
                                <span
                                    className={cn(
                                        ...
                                        isTodayDate
                                            ? "bg-[#0077b5] text-white shadow-md ring-2 ring-blue-50"
                                            : isSelectedMonth
                                ...
                                {onDateClick && !isBefore(day, startOfToday()) && (
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className={cn("opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-full hover:bg-blue-50 hover:text-[#0077b5]", mini ? "h-5 w-5" : "h-7 w-7")}
                                    ...
                                    return (
                                        <DropdownMenu key={post.id}>
                                            <DropdownMenuTrigger asChild>
                                                <div
                                                    className={cn(
                                                        ...
                                                        "bg-white hover:shadow-md hover:scale-[1.02] active:scale-95 border-gray-200",
                                                    ...
                                                    <div className={cn("relative flex-shrink-0 rounded-md overflow-hidden bg-gray-100 border border-gray-200", mini ? "w-4 h-4" : "w-7 h-7")}>
                                                        {mediaUrl ? (
                                                            ...
                                                            <div className="w-full h-full flex items-center justify-center bg-blue-50">
                                                                <LinkedinLogo className="h-4 w-4 text-[#0077b5]" />
                                                            </div>
                                                        )}
                                                        {isProcessing && (
                                                            <div className="absolute inset-0 bg-white/60 flex items-center justify-center">
                                                                <Loader2 className="h-3 w-3 animate-spin text-[#0077b5]" />
                                                            </div>
                                                        )}
                                                    </div>
                                            ...
                                            <DropdownMenuContent align="start" className="w-[540px] p-0 overflow-hidden border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.15)] bg-white rounded-[32px]">
                                                <div className="flex">
                                                    {/* Left side: LinkedIn post design */}
                                                    <div className="w-[320px] flex-shrink-0 bg-gray-50/50 p-4 border-r border-gray-100/50">
                                                        <LinkedinPreview
                                                            content={{
                                                                message: post.text || post.message || post.content?.text || post.caption || "",
                                                                media: media
                                                            }}
                                                            page={accounts.find(a => a.accountId === post.accountId) || { displayName: post.displayName || "LinkedIn" }}
                                                            compact={true}
                                                            hideLogo={true}
                                                            noBorder={true}
                                                        />
                                                        ...
                                                        {isPublished ? (
                                                            <>
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="flex flex-col">
                                                                        <span className="font-bold text-[13px] text-gray-900">Analytics</span>
