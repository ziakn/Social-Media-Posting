...
export async function GET(request) {
    ...
    try {
        ...
        return NextResponse.redirect(
            `${baseUrl}/admin/social/connect?status=success&platform=twitter&name=${encodeURIComponent(twitterUser.data.name)}`
        );
        ...
        const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
        return NextResponse.redirect(
            `${baseUrl}/admin/social/connect?status=failed&platform=twitter&message=${encodeURIComponent(error.message)}`
        );
    }
}
