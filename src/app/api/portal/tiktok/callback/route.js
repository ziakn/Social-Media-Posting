...
export async function GET(request) {
    try {
        ...
        if (!state || state !== storedState) {
            ...
            return NextResponse.redirect(`${baseUrl}/portal/social/connect?status=failed&platform=tiktok&message=Security check failed: State mismatch.`);
        }
        ...
        if (!code_verifier) {
            ...
            const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
            return NextResponse.redirect(`${baseUrl}/portal/social/connect?status=failed&platform=tiktok&message=Security check failed: Missing code verifier.`);
        }
        ...
        const response = NextResponse.redirect(
            `${baseUrl}/portal/social/connect?status=success&platform=tiktok&name=${encodeURIComponent(profileInfo.display_name)}`
        );
        ...
        return NextResponse.redirect(
            `${baseUrl}/portal/social/connect?status=failed&platform=tiktok&message=${encodeURIComponent(error.message)}`
        );
    }
}
