...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.redirect(
      `${baseUrl}/portal/social/connect?platform=instagram&status=success&connected=1&name=${encodeURIComponent(igUser.username)}`
    );
    ...
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
    return NextResponse.redirect(
      `${baseUrl}/portal/social/connect?platform=instagram&status=failed&message=${encodeURIComponent(
        err.message
      )}`
    );
  }
